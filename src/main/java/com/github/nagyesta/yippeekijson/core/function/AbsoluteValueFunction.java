...
/**
 * {@link java.util.function.Function} for calculating the absolute value of the node value.
 */
@Slf4j
public final class AbsoluteValueFunction extends DecimalFunctionSupport {

    static final String NAME = "absoluteValue";
    ...
    @SchemaDefinition(
            ...
            wikiLink = @WikiLink(file = WikiConstants.BUILT_IN_FUNCTIONS, section = "Absolute value function"),
            description = {
            ...
            example = @Example(
                    ...
                    out = "/examples/json/distances_out.json",
                    yml = "/examples/yml/distances_absolute.yml",
                    note = "In this example we have replaced the distance values with their absolute values."
            )
    )
    @NamedFunction(NAME)
    public AbsoluteValueFunction(@ValueParam(docs = "The number of digits we want to keep right of the decimal point.")
    ...
    protected BiFunction<BigDecimal, BigDecimal, BigDecimal> getFunction() {
        ...
        return (op1, ignore) -> op1.abs().setScale(scale, RoundingMode.HALF_UP);
    }
}
