...
public class SupplierComponentTypeDefinitionRegistry extends AbstractComponentTypeDefinitionRegistry
        implements JsonNamedComponentTypeDefinitionRegistry<Supplier<?>> {

    private static final String TYPE_DEFINITIONS_ROOT = "#/definitions/" + SUPPLIER_TYPES + "/definitions/";
    private static final String ANY_TYPE = "anySupplierType";
    private static final String CACHE_KEY = "'supplierDefinitions'";
    private static final String TYPE_NAME_MESSAGE_FORMAT = "{0}SupplierType";

    private final Map<Type, Set<String>> outputToRefMap = new TreeMap<>(Comparator.comparing(Type::getTypeName));
    private final Map<Type, String> outputToTypeNameMap = new TreeMap<>(Comparator.comparing(Type::getTypeName));

    public SupplierComponentTypeDefinitionRegistry(@NotNull final ComponentContextMetadataParser componentContextMetadataParser,
    ...
    private void putTypeRefs(final LinkedHashMap<String, JsonSchemaTypeDefinition> cachedMap) {
        outputToTypeNameMap.forEach((outputType, typeName) -> {
            Set<String> typeRefs = outputToRefMap.getOrDefault(outputType, Set.of());
    ...
    protected Class<? extends Annotation> getNamedAnnotation() {
        return NamedSupplier.class;
    }

    @Override
    @CacheEvict(cacheNames = JSON_TYPES, key = CACHE_KEY)
    public void registerType(@NotNull final Class<? extends Supplier<?>> namedType) {
        final ComponentContext componentContext = parseAndRegister(namedType);
        final Type output = translateKnownType(Optional.ofNullable(componentContext.getDocumentation())
                .flatMap(DocumentationContext::getOutputType)
                .orElse(Object.class));
        registerToRefTypes(output, componentContext.getJsonTypeName());
        registerToKnownTypeNames(output);
    }

    private void registerToRefTypes(@NotNull final Type output,
                                    @NotNull final String jsonTypeName) {
        this.outputToRefMap.computeIfAbsent(output, ignored -> new TreeSet<>())
                .add(jsonTypeName);
    }

    private void registerToKnownTypeNames(@NotNull final Type output) {
        if (Object.class.equals(output)) {
            return;
        }
        String outputName = StringUtils.uncapitalize(ParametrizedTypeAware.TYPE_NAME_TRANSLATION.get(output));
        this.outputToTypeNameMap.computeIfAbsent(output, ignored -> MessageFormat.format(TYPE_NAME_MESSAGE_FORMAT, outputName));
    }
}
